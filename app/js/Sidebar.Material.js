...
Sidebar.Material = function(editor) {
	...
	managerRow.add(new UI.Button('Paste').onClick(function() {
		...
		refreshUI(true, self.selectedMat)
		update();
	...
	function update() {
		...
		var material = self.selectedMat || object.material;
	...
	function setRowVisibility(mat = null) {
	...
	function multipleMatSelect() {
		const materials = currentObject.material.materials;
		const selectedUUID = multipleMaterialChoices.getValue()
		if (!materials || !selectedUUID)
			return

		const mat = materials.find(itm => {
			return itm.uuid == selectedUUID
		})
		console.log("MultiMaterial Select: ", selectedUUID, mat)
		self.selectedMat = mat

		currentObject._subMatID = selectedUUID
		refreshUI(true, self.selectedMat)
	}

	function refreshMultipleMat(currentObject) {
		var materials = currentObject.material.materials;
		let options = {}
		for (let index in materials) {
			let itm = materials[index]
			options[itm.uuid] = itm.name
		}
		multipleMaterialChoices.setOptions(options)
			// console.log(materials)
	}


	function refreshUI(resetTextureSelectors, mat = null) {
		...
		multipleMatRow.setDisplay('')
		if (material.type == "MultiMaterial") {
			multipleMatRow.setDisplay('')
			refreshMultipleMat(currentObject);
	...
	signals.objectSelected.add(function(object) {
		...
		if (object && object.material) {
			...
			if (currentObject && currentObject.material && currentObject.material.type == "MultiMaterial")
				...
				multipleMaterialChoices.setDisplay("none")
